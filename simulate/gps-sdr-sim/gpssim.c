...
double USER_TEMP_C = 15.0;     
double USER_PRESS_HPA = 1013.25;
double USER_HUMID_PCT = 50.0;   
 ...
 */
 ...
 // model Saastamoinena z opóźnieniem toposferycznym
double saastamoinen_model_exact(double el_rad) {
    double P = USER_PRESS_HPA;     
    double Tc = USER_TEMP_C;       
    double RH = USER_HUMID_PCT;   

    double Tk = Tc + 273.15;     
    const double PI_VAL = 3.14159265358979323846;
    double es = 6.112 * exp((17.67 * Tc) / (Tc + 243.5));
    double e = es * (RH / 100.0);
    if (el_rad < 0.087) el_rad = 0.087;

    double z_i = (PI_VAL / 2.0) - el_rad;

    double cos_z = cos(z_i);
    double tan_z = tan(z_i);

    double term_1 = 0.002277 / cos_z;
    double term_2 = P + ( (1255.0 / Tk) + 0.05 ) * e - (tan_z * tan_z);
...
void computeRange(range_t *rho, ephem_t eph, ionoutc_t *ionoutc, gpstime_t g, double xyz[])
{
	...
	// dodawanie opóźnienia troposferycznego
	double tropo_delay = saastamoinen_model_exact(rho->azel[1]);
	rho->range += tropo_delay;
...
int main(int argc, char *argv[])
{
	...
	while ((result=getopt(argc,argv,"e:u:x:g:c:l:o:s:b:L:T:t:d:ipvC:P:H:"))!=-1)
	{
		switch (result)
		{
		case 'e':
		...
		case 'v':
			...
			break;
		case 'C':
            USER_TEMP_C = atof(optarg);
            fprintf(stderr, "Ustawiono Temperature: %.1f C\n", USER_TEMP_C);
            break;
        case 'P':
            USER_PRESS_HPA = atof(optarg);
            fprintf(stderr, "Ustawiono Cisnienie: %.1f hPa\n", USER_PRESS_HPA);
            break;
        case 'H':
            USER_HUMID_PCT = atof(optarg);
            fprintf(stderr, "Ustawiono Wilgotnosc: %.1f %%\n", USER_HUMID_PCT);
            break;
		case ':':
		case '?':
			usage();
