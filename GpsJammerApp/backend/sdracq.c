...
extern uint64_t sdraqcuisition(sdrch_t *sdr, double *power) {
    ...
    if (sdr->ctype == CTYPE_G1 && !sdr->flagacq) {
        SDRPRINTF(
            "DEBUG GLONASS ACQ [%s]: Acquisition failed after %d attempts, "
            "peakr=%.2f, cn0=%.1f dB-Hz\n",
    ...
    if (sdr->flagacq) {
        ...
        if (sdr->ctype == CTYPE_G1) {
            SDRPRINTF("DEBUG GLONASS ACQ [%s]: Acquisition SUCCESS after %d "
                      "attempts, acqfreq=%.3f MHz\n",
...
extern int checkacquisition(double *P, sdrch_t *sdr) {
    ...
    if (sdr->ctype == CTYPE_G1) {
        SDRPRINTF(
            "DEBUG GLONASS ACQ [%s]: peakr=%.2f (threshold=%.2f), cn0=%.1f "
            "dB-Hz, acqfreq=%.3f MHz, codei=%d, freqi=%d\n",
