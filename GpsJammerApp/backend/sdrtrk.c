...
extern uint64_t sdrtracking(sdrch_t *sdr, uint64_t buffloc, uint64_t cnt)
{
    ...
    if (bufflocnow>buffloc) {
        ...
        int ctr;
        if (ctr%500000 == 0) {
          printf("currnsamp: %d, clen: %d, remcode: %.1f, codefreq: %.1f, f_sf: %.0f\n",
            sdr->currnsamp, sdr->clen, sdr->trk.remcode, sdr->trk.codefreq,
            sdr->f_sf);
        }
        ctr = ctr + 1;
        */
...
extern void dll(sdrch_t *sdr, sdrtrkprm_t *prm, double dt)
{
    ...
    /*
    printf("codefreq: %.1f, crate: %.1f, codeNCO: %.1f, carrFreq, %.1f\n",
          sdr->trk.codefreq,sdr->crate,sdr->trk.codeNco,sdr->trk.carrfreq);
    printf("f_if: %.1f, foffset: %.1f, f_cf: %.1f, codeErr: %.1f\n",
          sdr->f_if,sdr->foffset,sdr->f_cf,codeErr);
    */
}
