...
extern uint64_t sdrtracking(sdrch_t *sdr, uint64_t buffloc, uint64_t cnt) {
    ...
    if (bufflocnow > buffloc) {
        ...
        rcvgetbuff(&sdrini, buffloc, sdr->currnsamp, sdr->ftype, sdr->dtype,
                   data);

        memcpy(sdr->trk.oldI, sdr->trk.II,
               1 + 2 * sdr->trk.corrn * sizeof(double));
        memcpy(sdr->trk.oldQ, sdr->trk.QQ,
               1 + 2 * sdr->trk.corrn * sizeof(double));
...
extern void dll(sdrch_t *sdr, sdrtrkprm_t *prm, double dt) {
    ...
    codeErr = (sqrt(IE * IE + QE * QE) - sqrt(IL * IL + QL * QL)) /
              (sqrt(IE * IE + QE * QE) + sqrt(IL * IL + QL * QL));

    sdr->trk.codeNco +=
        prm->dllaw * (codeErr - sdr->trk.codeErr) + prm->dllw2 * dt * codeErr;

    sdr->trk.codefreq = sdr->crate - sdr->trk.codeNco +
                        (sdr->trk.carrfreq - sdr->f_if - sdr->foffset) /
                            (sdr->f_cf / sdr->crate);
...
extern void setobsdata(sdrch_t *sdr, uint64_t buffloc, uint64_t cnt,
    ...
    shiftdata(&trk->cntout[1], &trk->cntout[0], sizeof(uint64_t),
              OBSINTERPN - 1);
    shiftdata(&trk->remcout[1], &trk->remcout[0], sizeof(double),
              OBSINTERPN - 1);

    trk->tow[0] =
        sdr->nav.firstsftow + (double)(cnt - sdr->nav.firstsfcnt) * sdr->ctime;
    trk->codei[0] = buffloc;
    trk->cntout[0] = cnt;
    ...
    if (snrflag) {
        ...
        shiftdata(&trk->codeisum[1], &trk->codeisum[0], sizeof(uint64_t),
                  OBSINTERPN - 1);
